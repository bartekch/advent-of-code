...
### update - we could use simple subsetting for neighbours


countCubes4D2 <- function(initial_state, N = 6) {
  ...
  for (i in 1:N) {
    ...
    for (x in (N+2-i):(N+1+ni+i)) {
      for (y in (N+2-i):(N+1+ni+i)) {
        for (z in (N+2-i):(N+2+i)) {
          for (w in (N+2-i):(N+2+i)) {
            neigh <- sum(space[(x-1):(x+1), (y-1):(y+1), (z-1):(z+1), (w-1):(w+1)]) - space[x,y,z,w]
            if (space[x,y,z,w] && !neigh %in% 2:3) {
              new_space[x,y,z,w] <- FALSE
            } else if (!space[x,y,z,w] && neigh == 3) {
              new_space[x,y,z,w] <- TRUE
...
microbenchmark(countCubes4D(input), countCubes4D2(input), times = 10)
# three times faster

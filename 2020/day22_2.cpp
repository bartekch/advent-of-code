...
//[[Rcpp::export]]
IntegerVector playRecursiveCombat2Cpp(int maxit, Rcpp::IntegerVector pl1, Rcpp::IntegerVector pl2, int level) {
  ...
  while (l1 > 0 && l2 > 0 && i < maxit) {
    ...
    if (l1 > pl1_card && l2 > pl2_card) {
      winner = playRecursiveCombat2Cpp(maxit, head(pl1, pl1_card), head(pl2, pl2_card), level + 1)[0];
    } else {
      if (pl1_card > pl2_card) {
        winner = 1;
      } else {

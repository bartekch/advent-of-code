...
system.time(
  res_ex2_cpp_opt <- playCupsCppOpt(example2, maxit = 1e7)
)
# user  system elapsed
# 0.362   0.000   0.364
prod(res_ex2_cpp_opt[2:3])
all.equal(res_ex2_opt, res_ex2_cpp_opt)


system.time(
  res_run2_cpp_opt <- playCupsCppOpt(input2, maxit = 1e7)
)
# user  system elapsed
# 0.362   0.000   0.364
prod(res_run2_cpp_opt[2:3])
all.equal(res_run2_opt, res_run2_cpp_opt)



# speed comparison
bench <- microbenchmark::microbenchmark(
  playCupsOpt(input2, N = 1e7),
  playCupsCppOpt(input2, maxit = 1e7),
  times = 10
)
bench
ggplot2::autoplot(bench)
